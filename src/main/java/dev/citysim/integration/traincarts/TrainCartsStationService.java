...
public class TrainCartsStationService implements StationCounter {
    ...
    public TrainCartsStationService(JavaPlugin plugin) throws ReflectiveOperationException {
        ...
        this.signChunksField = resolveSignChunkField(signControllerWorldClass, longHashMapClass);
    ...
    private Field resolveSignChunkField(Class<?> worldClass, Class<?> longHashMapClass) throws NoSuchFieldException {
        try {
            return worldClass.getDeclaredField("signChunks");
        } catch (NoSuchFieldException ignored) {
            // Newer TrainCarts builds renamed the backing field; fall back to any LongHashMap field.
        }
        Field fallback = null;
        for (Field field : worldClass.getDeclaredFields()) {
            if (longHashMapClass.isAssignableFrom(field.getType())) {
                fallback = field;
                break;
            }
        }
        if (fallback == null) {
            throw new NoSuchFieldException("No LongHashMap sign storage field found on " + worldClass.getName());
        }
        return fallback;
    }
    ...
    private boolean isTrainOrCartHeader(String line) {
        String normalized = normalizeHeaderToken(line);
        return normalized != null && (normalized.equals("train") || normalized.equals("cart"));
    }
    ...
    private String normalizeHeaderToken(String line) {
        ...
        int start = 0;
        while (start < cleaned.length() && !Character.isLetterOrDigit(cleaned.charAt(start))) {
            start++;
        }
        ...
        while (end > 0 && !Character.isLetterOrDigit(cleaned.charAt(end - 1))) {
            end--;
        }
        if (end <= 0) {
            return null;
